...
def calculate_all_co2_mass(ecl, cellvolumes, initfile, summary, timestep, layer_mask):
    ...
    return np.sum(initfile["PORV"][layer_mask] * ecl["SGAS", timestep][layer_mask] * densityco2(pressure, temperature))
...
def calculate_qois(
    ...
    qoi_functions = {
        ...
        "pressure": lambda ecl, cellvolumes, initfile, summary, timestep, layer_mask: np.sum(
           cellvolumes[layer_mask] * ecl["PRESSURE", timestep][layer_mask] / np.sum(cellvolumes[layer_mask])
        ),
    }
    ...
    for qoi_name, qoi_function in qoi_functions.items():
        ...
        for ensemble_number in ensemble_numbers:
            ...
            if layer_mask is None:
                layer_mask = np.ones(cellvolumes.shape, dtype=np.bool)
