...
def plot_timeseries(foldername, errorevery=10):
    ...
    for qoi_name in ['real_mobile_co2', 'mobile_co2', 'all_co2', 'immobile_co2']:
        ...
        plt.errorbar(t, statistics_qoi['mean'], yerr=statistics_qoi['std'], errorevery=errorevery)
        plt.fill_between(t, statistics_qoi['min'], statistics_qoi['max'], alpha=0.6, color='salmon')
        ...
        plt.ylabel(f"$\\frac{{\\mathrm{{std}}({latex_name})}}{{\\mathbb{{E}}({latex_name})}}$")
        plt.xlabel('Time [years]')
        plt.tight_layout()
        plt.savefig(os.path.join(plot_folder, f'evolution_rel_std_{qoi_name}.png'), dpi=300)
        plt.close('all')
    ...
    plt.plot(t, np.array(statistics_qoi['std'])/np.array(statistics_qoi['mean']))
    plt.xlabel('Time [years]')
    plt.ylabel(f'$\\frac{{\\mathrm{{std}}({latex_names["diff"]})}}{{{latex_names["diff"]}}}$')
