...
def plot_timeseries(foldername, errorevery=10):
    ...
    timesteps = 215
    all_parameter_names = ['tegrad','pesand','peshale','pefeeder','posand','totopsur','dummy']
    latex_names = {
    ...
    for qoi_name in ['real_mobile_co2', 'mobile_co2', 'all_co2', 'immobile_co2', 'trapped_co2', 'pressure']:
        ...
        for ts in range(timesteps):
            ...
            for i in range(len(all_parameter_names)):
                M = len(all_qois[ts])
                statistics_qoi[f'corr_{all_parameter_names[i]}'].append(scipy.stats.pearsonr(parameters[:M,i], all_qois[ts]).statistic)
        ...
        for i in range(len(all_parameter_names)):
            plt.plot(t, statistics_qoi[f'corr_{all_parameter_names[i]}'], label=f'{all_parameter_names[i]}')
        plt.ylabel(f'Correlation')
        plt.xlabel('Time [years]')
        plt.title(f"Correlation for ${latex_names[qoi_name]}$")
        plt.text(50, 0, 'DRAFT!', fontsize=100, color='grey', rotation=45, 
        ...
        for k in ['S1',  'ST']:
            if f'{k}_{all_parameter_names[i]}' in statistics_qoi.keys():
                t = np.arange(0, min(timesteps, len(statistics_qoi[f'{k}_{all_parameter_names[i]}'])))
                for i in range(len(all_parameter_names)):
                    plt.plot(t, statistics_qoi[f'{k}_{all_parameter_names[i]}'], label=f'{all_parameter_names[i]}')
                    print(f"{k} ({qoi_name}) ({all_parameter_names[i]})= {statistics_qoi[f'{k}_{all_parameter_names[i]}'][-1]}")
                plt.ylabel(f'Sobol {k}')
                plt.xlabel('Time [years]')
                plt.title(f"Sobol {k} ${latex_names[qoi_name]}$")
                plt.text(100, 0, 'DRAFT!', fontsize=80, color='grey', rotation=30, 
                ha='center', va='center', alpha=0.5) 
                plt.legend()
                plt.tight_layout()

                plt.savefig(os.path.join(plot_folder, f'sobol_{k}_{qoi_name}.png'), dpi=300)
                plt.close('all')
